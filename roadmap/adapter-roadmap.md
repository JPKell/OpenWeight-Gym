...
| **LA3** | Route, on evidence | FreeWeight 1.1 (§4.3): adapter enumeration, panels + regression panel, serving-mode A/B, evidence export with adapter fields · LoadCoach routed selection live behind `require_adapter_evidence` · PromptCadence tiers select adapter subjects where evidence exists (§4.5). **FreeWeight's half is complete** (row H4, 2026-09-06, `docs/history/H4_HANDOFF.md`): every item above is built, green and demonstrated live on the reference machine. **LoadCoach's half is blocked on [ADR-0085](../adr/0085-the-evidence-uniqueness-key-carries-the-adapter.md)** — its uniqueness key must gain the adapter before there are records left to bind (§7, I18) | An imported bundle visibly changes which adapter a task profile selects, with the explanation showing per-capability adapter evidence beside the bare base; an unmeasured adapter is filtered with the named rejection; converges at or before PromptCadence P9 (M12) |
...
| **I18** | Evidence changes routing | LA3 | A FreeWeight bundle import flips a profile's selected subject to (or from) an adapter, visibly in the explanation, with no shared code or database. **Producer half done at H4** — a `benchmark.evidence_bundle` `1.1` carrying three subjects' records exports, validates with `setspec` alone, crosses as a file, and its bare-base record imports and binds. **Blocked on the consumer**, and on something one step earlier than expected: [ADR-0022 §3](../adr/0022-capability-evidence-record-contract.md)'s uniqueness key predates the adapter axis, so the two adapter-bearing records were **rejected as duplicates** rather than landing `unmatched` as row H2's handoff predicted. [ADR-0085](../adr/0085-the-evidence-uniqueness-key-carries-the-adapter.md) closes the decision; the consumer's row is now two changes — the key first, then the adapter axis on `capability_evidence` — and the key must move before there are records left to bind |
