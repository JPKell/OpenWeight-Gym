...
| Status | Row W2 (2026-09-09): `0.2.0` prepared, unpublished (`0.1.0` is on PyPI; `pip install wr-gym`). `wr-gym setup && wr-gym serve` gives an HTTPS console with a login and the audit trail; the five `systemd --user` units, start/stop/restart, the journal live and unified, the Ollama memory-safety pane and the audit page are here. The telemetry strip, the shell and the per-application pages are rows W3–W9 (`docs/roadmap/weightroom-work.md`). |
