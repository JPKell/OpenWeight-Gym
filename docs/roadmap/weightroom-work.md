# WeightRoom Work — the arc's schedule

**Started 2026-09-09** (row W0, from the operator interview of the same day). The fifth
application — WeightRoom, the host operator's console — in execution order, one row per model
...
* **WM2 after W10** — adoption targets a released MirrorWall 0.3 and a released WeightRoom.

## 4. Not model sessions — the human/ops checklist

* **Confirm the PyPI name.** `weightroom` is free and `weightroom` is taken (`0.0.1`, unrelated)
  as of 2026-09-09; reserve `weightroom` before W1 publishes anything.
* **Confirm the "htmx" reading** (ADR-0123 rule 7): the patterns through MirrorWall's modules, not
  the library. If the operator meant the library, that is an ADR superseding ADR-0020's rejection
  before W3, not a row.
...
* Per release: tag, Release-workflow `pypi` environment approval, post-publish install check —
  for `mirrorwall 0.3.0` (WM), the four `WS` patch releases, and `weightroom 1.0.0`.
...
| **WR-1.0** — `weightroom 1.0.0` | W8, W9, W10 | Every spec §20 criterion; the independent-device verification says *ready* |
