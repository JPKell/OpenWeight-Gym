...
| **WR-β** — the console operates | WS1–WS4, W4, W5, W6, W7 | Spec §20 #3–#6 and #8 demonstrated: a guarded write end to end, a schema-driven form with a live key and a re-authenticated security key, chat through both paths with an approval, an unknown revision degrading by name. **Every row done 2026-09-10; the operator declares it once `main` is pushed and CI's db-matrix job is green on GitHub** (red since W5, fixed at W7 gate B, not yet seen there — operator, 2026-09-10) |
