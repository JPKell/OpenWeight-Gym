...
|---|---|---|---|---|---|
| **W0** — **done 2026-09-09** (`history/W0_HANDOFF.md`; no code) | **WeightRoomGym Phase 0** → ADRs 0123–0127, `apps/weightroom/{spec,api,data-model,design,risks,development-plan}.md`, the repository move (`~/ai/suite/docs` → `~/ai/suite/WeightRoom`, docs under `docs/`, a `docs` symlink for one transition), the skeleton at `0.0.0` with the gate green, master architecture and its satellites amended, `MEMORY_SAFETY.md` §2.2 and `LAN_ACCESS.md` rewritten, `expose_on_lan.sh` deleted, this file and the fifteen kickoff prompts below | **Fable 5.1 · attended** | N6 | The interview as decisions; the PyPI name confirmed against the index and then decided by the operator: product WeightRoomGym, distribution and CLI `wr-gym` (`weightroom` is taken); the one interpretation flagged for the operator is "htmx" (ADR-0123 rule 7: the patterns, not the library). **Read:** `history/w0-weightroom-phase-0.prompt.md` | Judgment throughout — the boundary exception, the write guard, the CA, the schema verbs; the operator wanted to be present |
