...
* **P7 — adapters.** *(Built 2026-09-04 — `docs/history/handoffs/F3_HANDOFF.md`.)* Launch-time registration from
...
| **I17** | Cache correctness | LA1 | The prefix-under-A-never-reused-for-B test, plus a semantic canary (same prompt, two adapters, distinct outputs after a shared prefix). **Structural half done at P7** and proved by making it fail against three injected defects; the **canary is blocked on artefacts** — no LLM adapter GGUF exists on the reference machine, and it skips visibly naming what it needs (`docs/history/handoffs/F3_HANDOFF.md`). Producing two adapters for one base is an **H1 prerequisite**, alongside I16's twenty-generation run |
| **I18** | Evidence changes routing | LA3 | A FreeWeight bundle import flips a profile's selected subject to (or from) an adapter, visibly in the explanation, with no shared code or database. **Passed whole at H5, 2026-09-06** (`docs/history/handoffs/H5_HANDOFF.md`; `FreeWeight/tests/live/test_la3_adapters.py` + `LoadCoach/tests/live/test_la3_evidence.py`). A `benchmark.evidence_bundle` `1.1` carrying three subjects' records was exported by FreeWeight, carried as a **file** — no shared code, no shared database — and imported through `loadcoach evidence import`: **three records, zero rejections, three subjects bound**, each to its own subject and none to the base. Routing then selected `+verbose` on a `benchmark` signal naming the imported evidence, while `pirate` — measured nowhere — was rejected `adapter_unmeasured` in the same decision. Getting there took two amendments neither handoff predicted: [ADR-0086](../adr/0086-the-consumers-adapter-key-column-is-not-nullable.md) (the consumer's key column is not nullable, because an `ON CONFLICT` target never fires on a `NULL`) and [ADR-0074](../adr/0074-adapter-enabled-serving-is-a-runtime-profile-field.md) rule 3 finally implemented in FreeWeight, without which every LA3 measurement carried a `runtime_profile_hash` that never happened and was excluded by the consumer as `evidence_profile_mismatch` |
| **I19** | Classification invariant | LA2 | A confidential-classified adapter with a remote-tier request produces a recorded denial; the turn/attempt records the adapter's classification. **The denial is a LoadCoach routing rejection** — an `adapter_classification_conflict` row in `routing_candidates`, queryable through the explanation — and **not** a `governance.egress_decision`, which is written only by an application holding a Commissioner ledger about a request it meant to send ([ADR-0079](../adr/0079-an-adapter-classification-refusal-is-a-routing-rejection.md), row H2) |
