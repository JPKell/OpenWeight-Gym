# Kickoff — W1: WeightRoomGym Phase 1 — skeleton, configuration, database, TLS, login, `setup`

**Row:** W1 (Opus 5 · xhigh · **never overnight**) — row 2 of
[`docs/roadmap/weightroom-work.md`](../../roadmap/weightroom-work.md). Runs after W0.
**Ships:** `wr-gym 0.1.0` prepared (not published): `wr-gym serve` on HTTPS with a
login, `wr-gym setup` minus the units step, the audit log, the full CI shape with
...
`history/handoffs/W0_HANDOFF.md` (what the skeleton deferred and why). Precedents: LoadCoach's
...
reference|schema` (the schema verb is ADR-0127 rule 1 applied to WeightRoomGym itself), the
...
`wr-gym trust`; uvicorn with `ssl_certfile`/`ssl_keyfile`; the trust listener as a second,
...
**Gate D — `setup`, health, shell, CI.** `wr-gym setup` (TLS, account, `allowed_hosts` from
...
`/api/v1/health` is `401`. Put the phone's certificate details and `wr-gym tls show` side by
side in the handoff.
...
gate, `docs/history/handoffs/W1_HANDOFF.md` written (decisions, what the kickoff got wrong, the seeded
