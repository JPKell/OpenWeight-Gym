...
**Runs after:** G1 (done 2026-09-04, `docs/history/handoffs/G1_HANDOFF.md`). **Runs before:** I2's sandboxed-tool
...
* **Ollama is installed and running** with gpt-oss:20b among the pulled models — the model G1
  ...
  G1 found four CLI tests read a live server if one is up (`docs/history/handoffs/G1_HANDOFF.md` §14.6).
...
*sandboxed tools* clause **unmet**, for a reason outside that row. `docs/history/handoffs/G1_HANDOFF.md` §9.3 and the
...
1. **`docs/history/handoffs/G1_HANDOFF.md` §9 and §10.4 in full** — the evidence. §10.4 is the transcript this row makes
...
* Tests: an assistant turn with calls and **no content** now replays (this is the exact turn that
  broke G1 — `docs/history/handoffs/G1_HANDOFF.md` §10.4, `turn 3 s1`); a `tool` turn answering it by `tool_call_id`
...
1. **The fake gains the same fields** (`tests/fakes/loadcoach_app.py`), with the real refusals
   ...
   produce before (`docs/history/handoffs/G1_HANDOFF.md` §14.6). Update
...
5. **The demonstration, verbatim into the handoff.** Real LoadCoach `1.0.0` (this working tree) on
   ...
   `PROMPTCADENCE_TIERS__*` overrides** — the same conditions as `docs/history/handoffs/G1_HANDOFF.md` §10. Run
...
6. Note for I2/P9, do not fix here: natively-replayed `arguments` are **still uncapped model
   ...
   `docs/history/handoffs/G1_HANDOFF.md` §8 flagged; deleting it does not remove the hazard, it moves it. Write it up.
