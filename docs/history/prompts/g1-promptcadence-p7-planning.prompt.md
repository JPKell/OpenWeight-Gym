...
* **`tools.plan` is a real, shipped LoadCoach profile** — E4 added the five harness profiles and a
  ...
  (`docs/history/handoffs/E4_HANDOFF.md` §3). You are not inventing the planning profile; you are calling it.
...
(`docs/history/handoffs/F2_HANDOFF.md` §11): G1 changes *who mints the intent*, not what a turn is checked against.
...
4. **The surfaces.** `POST /trajectories/{id}/approve|deny`, `GET /approvals`,
   ...
   one to this row deliberately (`docs/history/handoffs/E4_HANDOFF.md` §4). `web/routes/system.py:109` hard-codes
...
6. Handoffs, for what they left you: **`docs/history/handoffs/F2_HANDOFF.md` §11** (what G1 inherits, and why the
   deviation matrix golden is the invariance baseline), **`docs/history/handoffs/F4_HANDOFF.md` §11–12** (what G1 must
   not relitigate about the ledger, and the per-tier column it can now build),
   **`docs/history/handoffs/D2_HANDOFF.md` §3–4** (the bypass loop, leases, CAS fencing, and the T2→T7 stub you replace),
   **`docs/history/handoffs/E4_HANDOFF.md` §3–4** (the five task profiles; `tiers check` left to you).
...
* Per `docs/history/handoffs/F2_HANDOFF.md` §11, every turn already carries an egress decision in both modes, and
...
* **`GET /system/status` loses its placeholder**: `pending_approvals` with ages, and today's ledger
  ...
  them — `docs/history/handoffs/F4_HANDOFF.md` §11–12 says exactly what not to re-derive: **a tier has a balance, not
...
2. **`docs/history/handoffs/G1_HANDOFF.md` at the workspace root**, house shape: the gate results; the §0.3 decision and
...
* **Do not add a field to `TurnFacts`** or a new deviation category; both are governance dimensions
  needing an ADR (`docs/history/handoffs/F2_HANDOFF.md`'s stop rule, still in force).
