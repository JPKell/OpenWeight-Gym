# Kickoff — WS4: PromptCadence 1.3.3 — `config schema --json` and `config validate --file`

**Row:** WS4 (Sonnet 5 · high; overnight allowed) — [`docs/roadmap/weightroom-work.md`](../../roadmap/weightroom-work.md).
Runs after W0; independent of W1–W3 and of the other three WS rows; **before W4**.
**Ships:** `promptcadence 1.3.3` prepared (not published): two verbs on the existing `config` group, a
golden of the schema document, CHANGELOG, the mirror.
**Component:** `~/ai/suite/PromptCadence` (its `.venv`); then one docs commit in `~/ai/suite/WeightRoom`
(`apps/promptcadence/spec.md` §7.2 gains the two verbs — edit the canonical copy first, then
`cmp`-prove the mirror in `PromptCadence/docs/`; the row marked done).
...
`apps/promptcadence/spec.md` §7.2 and §12; `standards/configuration-standards.md` §4, §7, §8;
`standards/cli-standards.md` §3 (`--json` output), §9; PromptCadence's `config.py` (the `Settings`
...
* **The registry is ADR-0100's five keys**, and the refused set is whole sections (`[server]`, `[loadcoach]`, `[approval]`, `[budget]`, `[tools]`, `[tiers]`, `[policy]`) plus named keys — `security_keys` lists every leaf those sections expand to, not the section names, so a form can mark each field.
* `sources` carries `shadowed_by` exactly as `GET /settings` already reports it.
...
**Gate C — release-prepared.** `CHANGELOG.md`, `__about__` `1.3.3`, the generated
...
`promptcadence config schema --json | python -m json.tool | head -60` on the reference machine against
the operator's real configuration, and `promptcadence config validate --file /tmp/candidate.toml` on a
...
`docs/history/handoffs/WS4_HANDOFF.md` naming the document's exact shape and any quirk W4 must know.
