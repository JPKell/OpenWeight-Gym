# Kickoff — WS1: FreeWeight 1.2.1 — `config schema --json` and `config validate --file`

**Row:** WS1 (Sonnet 5 · high; overnight allowed) — [`docs/roadmap/weightroom-work.md`](../../roadmap/weightroom-work.md).
Runs after W0; independent of W1–W3 and of the other three WS rows; **before W4**.
**Ships:** `freeweight 1.2.1` prepared (not published): two verbs on the existing `config` group, a
golden of the schema document, CHANGELOG, the mirror.
**Component:** `~/ai/suite/FreeWeight` (its `.venv`); then one docs commit in `~/ai/suite/WeightRoom`
(`apps/freeweight/spec.md` §7.2 gains the two verbs — edit the canonical copy first, then
`cmp`-prove the mirror in `FreeWeight/docs/`; the row marked done).
...
`apps/freeweight/spec.md` §7.2 and §12; `standards/configuration-standards.md` §4, §7, §8;
`standards/cli-standards.md` §3 (`--json` output), §9; FreeWeight's `config.py` (the `Settings`
...
* FreeWeight's `RUNTIME_SETTINGS` is a tuple of `RuntimeSetting` with `kind` as a string; the document's `runtime_changeable` entries carry `key`, `kind`, `minimum`, `maximum`, `description` in the one shape all four rows emit (ADR-0127 rule 1's example) — map the field names, do not leak the internal ones.
* The `[provider]` block edited by ADR-0117's `PUT /provider` is part of `json_schema` like any other; the document does not special-case it.
...
**Gate C — release-prepared.** `CHANGELOG.md`, `__about__` `1.2.1`, the generated
...
`freeweight config schema --json | python -m json.tool | head -60` on the reference machine against
the operator's real configuration, and `freeweight config validate --file /tmp/candidate.toml` on a
...
`docs/history/handoffs/WS1_HANDOFF.md` naming the document's exact shape and any quirk W4 must know.
