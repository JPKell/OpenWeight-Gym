...
decision above is unsettled: **stop there.** Write `docs/history/handoffs/E2_HANDOFF.md`, say exactly where you stopped, and
do not start E1.
...
* **The isolation ladder's container rung runs under docker here, and the bwrap rung is reachable**
  through D1's injected `which` seam (`docs/history/handoffs/D1_HANDOFF.md` §3). Both rungs stay exercised by the suite you
...
* **Podman is still unverified**, and row E2 requires `pytest -m isolation -rs` on a podman host
  *before* `0.1.0` publishes (`docs/history/handoffs/D1_HANDOFF.md` §13 row J). That cannot be done here. It is an operator
  step, it belongs in `docs/history/handoffs/E2_HANDOFF.md`'s "Before the next session", and it is a **publish blocker**,
  not a nice-to-have.
...
5. **`docs/history/handoffs/D1_HANDOFF.md` §7 and §8**, written for this row. §7 is what you must not relitigate; §8 is the
...
* **`run_command_tool(sandbox: Sandbox)`** — the *same* instance the executor holds (spec §7 as
  amended, `docs/history/handoffs/D1_HANDOFF.md` §13 row A). It declares `requires_isolation = True`, argv only, never a
...
3. [ADR-0052](docs/adr/0052-compaction-is-a-view-and-the-package-plans-it-only.md) (D-8) — the rule
   ...
   caller fulfils.
4. **`docs/history/handoffs/C1_HANDOFF.md` §3 and §4** — written for this row, and **not to be relitigated**: the
...
1. **`docs/history/handoffs/E2_HANDOFF.md`** and **`docs/history/handoffs/E1_HANDOFF.md`** at the workspace root. **Never overwrite an existing
...
2. **`docs/history/handoffs/E2_HANDOFF.md` additionally carries**: what the red-CI fix changed and why that answer rather
...
3. **`docs/history/handoffs/E1_HANDOFF.md` additionally carries**: the chain's projection design in enough detail that I1
