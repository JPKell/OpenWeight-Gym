...
* **The row's own text has one thing wrong, and you should not propagate it.** It says a failed
  ...
  `lifecycle.md:441`–`:442`, and G1's live halt at `docs/history/handoffs/G1_HANDOFF.md` §10.4 came out `halted` with
...
* **Python 3.13.15; there is no python3.12 on this host.** Name the interpreter and every exact
  ...
  tests read a live server if one is up (`docs/history/handoffs/G1_HANDOFF.md` §14.6).
...
* **Both paths.** The bypassed loop's synthetic step retries the same way, or contract 1 fails —
  ...
  `tests/contract/test_governance_invariance.py`, allowance list closed at `docs/history/handoffs/G1_HANDOFF.md` §5).
...
recommendation is (a) unless reading I1's needs says otherwise. **Read `docs/history/handoffs/G1_HANDOFF.md` §8's hazard
...
* `pytest-randomly` is on; a seed-only failure is a real bug, not a flake. G1's recovery race was
  found exactly that way (`docs/history/handoffs/G1_HANDOFF.md` §14.7).
...
3. **`docs/history/handoffs/G1_HANDOFF.md`**: **§9** (what the real stack found — the failures a retry would actually have
...
* Counted against `max_turns_per_step`. A step that exhausts its turns is `turn_overrun` and that
  ...
  loop-level test today (`docs/history/handoffs/G1_HANDOFF.md` §15c) and this row is the first thing that could confuse
  it.
...
1. **Contract 1 green with no new allowance.** `tests/contract/test_governance_invariance.py`. If
   ...
   allowance list to pass (`docs/history/handoffs/G1_HANDOFF.md` §5: the list is closed, a new difference is a finding).
...
* **Do not widen the contract-1 allowance list** to make the diff pass (`docs/history/handoffs/G1_HANDOFF.md` §5).
...
* **Do not relitigate G1's closed decisions** (`docs/history/handoffs/G1_HANDOFF.md` §13): the bypass grant supersedes;
...
`docs/history/handoffs/G1_HANDOFF.md` §15c) and what the cheapest such journey is; (c) whether anything in §0.2's
