# Kickoff — WS2: LoadCoach 1.3.1 — `config schema --json` and `config validate --file`

**Row:** WS2 (Sonnet 5 · high; overnight allowed) — [`docs/roadmap/weightroom-work.md`](../../roadmap/weightroom-work.md).
Runs after W0; independent of W1–W3 and of the other three WS rows; **before W4**.
**Ships:** `loadcoach 1.3.1` prepared (not published): two verbs on the existing `config` group, a
golden of the schema document, CHANGELOG, the mirror.
**Component:** `~/ai/suite/LoadCoach` (its `.venv`); then one docs commit in `~/ai/suite/WeightRoom`
(`apps/loadcoach/spec.md` §7.2 gains the two verbs — edit the canonical copy first, then
`cmp`-prove the mirror in `LoadCoach/docs/`; the row marked done).
...
`apps/loadcoach/spec.md` §7.2 and §12; `standards/configuration-standards.md` §4, §7, §8;
`standards/cli-standards.md` §3 (`--json` output), §9; LoadCoach's `config.py` (the `Settings`
...
* **Both provider forms appear.** `Settings` carries the singular `[provider]` block and the plural `[providers.<name>]` registry (ADR-0077); `json_schema` describes both, and the document says which form the file on disk uses (`"provider_form": "singular" | "plural"`) so a form generator does not render two editors for one registration. Record the exact shape in the handoff — W4 reads it.
* `sources` reports a stored row shadowed by the environment as such (row I8's fix; ADR-0100 decision 5's shape).
...
**Gate C — release-prepared.** `CHANGELOG.md`, `__about__` `1.3.1`, the generated
...
`loadcoach config schema --json | python -m json.tool | head -60` on the reference machine against
the operator's real configuration, and `loadcoach config validate --file /tmp/candidate.toml` on a
...
`docs/history/handoffs/WS2_HANDOFF.md` naming the document's exact shape and any quirk W4 must know.
