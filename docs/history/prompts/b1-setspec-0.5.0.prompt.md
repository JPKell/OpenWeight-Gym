...
1. **Write `docs/history/handoffs/B1_HANDOFF.md` at the workspace root.** It is read by the human before B3 is kicked
   ...
   **Never overwrite an existing root file** — the workspace root is not a git repository and has
   no history to recover from. If `docs/history/handoffs/B1_HANDOFF.md` exists, write `B1_HANDOFF.2.md` and say why.
...
* If you must stop early, stop at a green gate with a commit, leave the tree clean, and record
  exactly where you stopped in `docs/history/handoffs/B1_HANDOFF.md`.
