# WA1 handoff — SetSpec 0.7.0 + FreeWeight 1.3.0: `adapters_registered` on the wire and in the table
...
**Ships, prepared only:** `setspec 0.7.0`, `freeweight 1.3.0`, and WeightRoomGym's migration `0005`
on the unreleased `0.7.0` line. Nothing is tagged, pushed or published.
...
| SetSpec | `8d08f3c` | Gate A. `RuntimeProfileV1_1Fields`, `BenchmarkResultV1_1Fields`, `BenchmarkRunSummaryV1_1Fields` and their `V1_1Out`/`V1_1In` pairs; registration; two JSON Schemas; six goldens; `SUPPORTED_SCHEMAS` at `1.1`; contract and unit tests; `baseaicore>=0.4.2`; CHANGELOG; `0.7.0` |
| FreeWeight | `50dc05a` | Gate B. Migration `0010` and its backfill; the model column; the repository and `_stored_runtime_profile` carry the field; the export on `1.1`, `freeweight.export` versioned by content; tests; `setspec>=0.7`; OpenAPI snapshot (version string only); data model and spec mirrored; CHANGELOG; `1.3.0` |
...
| WeightRoom | this commit | Gate C, the docs: `packages/setspec/schemas.md` and `spec.md`, `apps/freeweight/data-model.md` and `spec.md` (canonical), the two synced `guide/README.md` copies, the row marked done, this handoff |
...
pytest -m "not live and not performance" --cov
                           # 1204 passed, 4 skipped; coverage 97.49 % (floor 95 %)
```
...
`freeweight 1.2.1`'s code on the editable `setspec 0.7.0`) failed the way the audit said:
...
* **Publish order.** `baseaicore 0.4.2` is already on PyPI. `setspec 0.7.0` first; then FreeWeight's
  `requirements/ci.lock` moves off `setspec==0.6.0` and `freeweight 1.3.0` can release. **FreeWeight's
  CI is red until then** (its `pyproject.toml` says `setspec>=0.7` with a `TODO: re-pin on publish`).
  WeightRoomGym's migration `0005` rides its next release, held with the rest of the arc.
* **Your real FreeWeight database is still at `0009`.** Installing `freeweight 1.3.0` and running
