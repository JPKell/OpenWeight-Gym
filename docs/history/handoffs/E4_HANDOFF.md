...
* **`toolyard>=0.1,<0.2`** added to `pyproject.toml`'s runtime dependencies, replacing the Phase-1
  ...
  PromptCadence has **no `requirements/` lock** (that is still its own row, per `docs/history/handoffs/D2_HANDOFF.2.md`
...
* **Everything in `docs/history/handoffs/D1_HANDOFF.md` §7 and `docs/history/handoffs/E2_HANDOFF.md` §8 still stands, and this row changed
...
3. **Podman is still unverified** (`docs/history/handoffs/D1_HANDOFF.md` §13 row J). This host has docker and no podman;
...
* **PromptCadence has no `requirements/` lock and no `security` job** — still its own row
  (`docs/history/handoffs/D2_HANDOFF.2.md` §7).
...
assertion was already decided on the row or in `docs/history/handoffs/D1_HANDOFF.md` §8/§13 and `docs/history/handoffs/D2_HANDOFF.2.md` §4/§7,
...
* **Never `git add -A`**; named paths only. No workspace-root file was overwritten — `docs/history/handoffs/E4_HANDOFF.md`
  is new.
