...
5. **Still open from B4:** reserve the PyPI name `promptcadence` (unverified), and the tier-defaults
   judgment call in `docs/history/handoffs/B4_HANDOFF.md` §3 (unchanged this session).
...
`docs/history/handoffs/B4_HANDOFF.md` §1 asked whether the siblings' `db-matrix` jobs are green, on the theory that they
