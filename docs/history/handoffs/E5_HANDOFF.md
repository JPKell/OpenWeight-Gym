...
`docs/history/handoffs/E4_HANDOFF.md` §8 and the kickoff §0 both list four unpushed repositories. After `git fetch` on
...
* **Do not** build PromptCadence's `ci.lock` or `security` job, and do not add IdeaPress's missing
  `ci.lock`. Both are recorded as their own future work (`docs/history/handoffs/D2_HANDOFF.2.md` §7, `docs/history/handoffs/E4_HANDOFF.md` §9).
...
1. E4's owed pushes for `docs`, `LoadCoach` and `PromptCadence` were **already done**; the SHAs in
   §0 and in `docs/history/handoffs/E4_HANDOFF.md` §8 are stale. Only MirrorWall was ahead. (§4)
