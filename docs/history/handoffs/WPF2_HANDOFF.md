...
| 2 | **Both halves are real, and one of them is FreeWeight's.** Two `llama-server` launches were caught live in the same sitting: the first (port **8180**) carried `--model …Qwen2.5-1.5B-Instruct.Q8_0.gguf … --lora …terse.gguf --lora-init-without-apply **--ctx-size 8192 --fit off**; the one still warm after the repeat (port **8181**, started 22:23:43 PDT in the repeat's own window) carried **neither `--ctx-size` nor `--fit off`** and answers `/props` with **`n_ctx` 32768, 4 slots**. Every one of the three runs recorded `served_context 8192 / configured`, so the degradation is right to fire and the run whose server had no flag was measured at 32 768. **A launch path still loses the runtime profile** — this row fixed FreeWeight's own Runs form, and something on the repeat/warm path is still building none. That is the alternative the kickoff's finding 4 named, and it is in FreeWeight, not only in ModelRack's `read_served_context`; it needs a row of its own. The degradation's new wording is what made the difference legible: it names both possibilities instead of claiming an assumption |
