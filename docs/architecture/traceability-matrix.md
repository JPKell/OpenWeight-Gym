...
| Data classification and its ordering | **BaseAiCore** | PromptCadence, Commissioner, adapter manifests, IdeaPress (M13) | Python API (`DataClassification`) | [ADR-0046](../adr/0046-data-classification-is-ordered-and-defaults-closed.md) |
...
| Egress evaluation and the decision ledger | **Commissioner** | PromptCadence, IdeaPress (M13) | Python API + SetSpec `governance.egress_decision` | [ADR-0054](../adr/0054-commissioner-records-egress-it-does-not-enforce-it.md) |
| Mountable persistence models | **LoadLedger, Commissioner** (shapes) / the host application (engine, history, data) | PromptCadence, IdeaPress (M13) | Python API (`mount_*_tables`) | [ADR-0050](../adr/0050-a-package-may-ship-tables-never-a-migration-history.md) |
...
| The operator's console: LAN edge, TLS, login, audit trail | **WeightRoomGym** | Operator | HTTPS (session) | [ADR-0126](../adr/0126-weightroom-is-the-only-service-on-the-lan-and-terminates-tls-with-its-own-ca.md) · [WeightRoomGym §14](../apps/weightroom/spec.md) |
| Process control and the unified log | **WeightRoomGym** (reads/drives all four) | Operator | `systemd --user`, journal | [ADR-0125](../adr/0125-weightroom-drives-the-applications-through-systemd-user-units-it-writes.md) |
| Settings forms over each application's schema | **WeightRoomGym** (each app publishes `config schema`) | Operator | CLI JSON document + in-place TOML | [ADR-0127](../adr/0127-every-application-publishes-its-settings-schema-and-weightroom-generates-the-form.md) |
| Guarded writes into another application's database | **WeightRoomGym** (reads FreeWeight, LoadCoach, IdeaPress, PromptCadence) | Operator | read-only DB; the five-part guard | [ADR-0124](../adr/0124-a-raw-write-into-another-applications-database-passes-a-five-part-guard.md) |
| Chat through the governed paths, catalog, costs, backups, jobs, alerts, prompts | **WeightRoomGym** (over LoadCoach's and PromptCadence's APIs) | Operator | HTTP v1 of each application | [WeightRoomGym §7](../apps/weightroom/spec.md) |
...
| Who owns **data classification** — an application, or a shared package? | BaseAiCore owns the type and its ordering; Commissioner compares; the applications declare. A per-application enum with boundary mappings was rejected because the mapping that quietly widens is invisible to both sides ([ADR-0046](../adr/0046-data-classification-is-ordered-and-defaults-closed.md)) |
