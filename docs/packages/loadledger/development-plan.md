...
* `position()` on the same three, alongside it: `remaining` for **no particular run** — every
  ...
  gap `docs/history/handoffs/F1_HANDOFF.md` §7 names — it fixes `--scope tier`, but `--scope day` and `--scope project`
...
PromptCadence needed for its per-tier and per-day views (`docs/history/handoffs/F1_HANDOFF.md` §7). The two ways to
