...
| Stream truncated without a terminal chunk | `ProviderProtocolError` | Partial result preserved. The message is literal — the server ended a well-formed NDJSON stream with no `done: true` line and no error object of its own, which is a fact about the server rather than about the parser. Measured on Ollama `0.32.13`: `gpt-oss:20b` under `think: false` **and** `format: "json"` closes this way in 12 of 13 requests straight to Ollama — most with no error object at all, some with `{"error": "error parsing tool call: …"}` — which this adapter surfaces as the provider's own message instead (I6, 2026-09-07; the run log is in [`history/handoffs/I6_HANDOFF.md`](../../history/handoffs/I6_HANDOFF.md) §4) |
