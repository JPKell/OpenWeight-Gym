...
| G6 | Every **transferable** payload is versioned, and every HTTP API is versioned in its path | Schema-version present on exports, bundles, results, profiles and event frames; unknown major rejected, unknown minor accepted with preservation; API *surface* contracted by the committed OpenAPI snapshot and API *bodies* by goldens captured from a running producer ([ADR-0108](../adr/0108-the-snapshot-contracts-the-surface-and-goldens-contract-the-bodies.md)). The membership test is [ADR-0025](../adr/0025-envelope-boundaries.md) | Contract tests, both directions |
| G7 | Every public API is documented | OpenAPI snapshot committed at `docs/openapi.json` and compared byte for byte with what the application serves; a change without a changelog entry fails | CI, blocking |
...
* **The exception is the list.** Its reach into the four applications is exactly
  ...
  `commissioner`.
* **Every state-changing action writes an audit row** — a test enumerates the routes and asserts
  one row each; the trail is append-only.
* **A raw write into another application's database passes the five-part guard or does not
...
* Serves the shell, the docs, the doctor and the audit trail with every application stopped, no
  systemd, no GPU and no network.
