...
| `action` | text | `unit.start`, `settings.write`, `db.guarded_write`, `db.curated`, `catalog.pull`, `prompt.override`, `job.run`, `alert.ack`, `login`, `logout`, `tls.rotate`, … a closed vocabulary asserted by test |
