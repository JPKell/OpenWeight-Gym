...
| `GET /apps/{app}/db/backups` | The guarded-write backups under WeightRoomGym's own `<data>/backups/<app>/`, newest first; never rotated, each the undo of one raw write |
