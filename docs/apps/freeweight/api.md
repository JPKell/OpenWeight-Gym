...
| `GET /goals/{slug}/calibration/report` | `kappa_w`, `rho`, `mae`, `bias`, `n_anchor`, `n_holdout`, inter-juror alpha, per criterion and weighted; gate verdict; `judge_validity_factor`; the worst-diverging holdout samples with both rationales. `n_holdout` is the samples a coefficient was **counted** over; beside it, `n_judged` (goal-level and per criterion) is how many the jury actually judged, and `excluded` names every judged sample not counted, with why (`self_judging`, `protocol_error`, `output_truncated`, `timeout`, `unparsed_grade`, or `not_graded_by_author` — `output_truncated` is a juror that hit its output limit without ever reaching an answer, which is a different event from `protocol_error`'s juror that answered something unusable, ADR-0141) — `n_judged` equals `n_holdout` and `excluded` is empty when nothing was dropped |
