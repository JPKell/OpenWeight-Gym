...
| [0141](0141-a-juror-that-runs-out-of-output-budget-is-named-not-a-protocol-error.md) | A juror cut off at its output limit refuses `output_truncated`, not `protocol_error`; a judge call is bounded by `[judge] max_output_tokens` (default 2048); FreeWeight never asks a reasoning juror not to reason, and eligibility stays a question of permission — the calibration report is what measures competence, and it says so in words | Accepted |
