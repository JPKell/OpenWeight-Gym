...
**WeightRoomGym is the fifth application, and it is the operator's tool: it sits above the layer
rules by an exception scoped exactly as wide as this record says, and it never imports an
application.**

1. **A fifth application, in every ordinary respect.** Product name **WeightRoomGym**, import
   name `weightroom`, distribution **and** CLI **`wr-gym`** (the operator's choice of
   2026-09-09; `weightroom` is held on PyPI by an unrelated `0.0.1`, and
   [Packaging Standards §8](../standards/packaging-and-release-standards.md)'s fallback rule is
   applied with the operator's name rather than `aisuite-weightroom`; the config, data and state
   roots are `wr-gym` to match the command, the environment prefix stays `WEIGHTROOM_` with the
   import name), default port
...
2. **Above the layer rules, by enumeration.** WeightRoomGym may, and no other component may:
   ...
   * import the suite's packages as any application does — `baseaicore`, `setspec`, `weightsdb`,
   ...
   holds every one of these rights at a shell, and WeightRoomGym exercises them on the operator's
...
3. **It never imports an application.** `.importlinter` forbids `freeweight`, `loadcoach`,
   ...
   exactly as the packages are forbidden. An import would bind WeightRoomGym to each application's
   ...
   application's pages to *schema not known to this WeightRoomGym*, by name, never a crash and never
   a guess. The four applications keep migrating freely; WeightRoomGym follows in its next release.
...
4. **It runs no tool, compacts nothing, and decides no egress of its own.** `toolyard`, `cutctx`
   ...
   the budget and the egress verdict; WeightRoomGym shows those decisions and never makes them. A
...
6. **The four applications are untouched in posture.** Each keeps its loopback bind, its own UI,
   its own tokens and its own security page. WeightRoomGym **re-implements** every control surface
...
8. **The look is the console's, and MirrorWall carries it.** WeightRoomGym is dense and dark-first
...
list, not a mood, so a reviewer can answer "may WeightRoomGym do X" by reading rule 2.

*Negative.* WeightRoomGym couples to four schemas and four CLIs. Rule 3's revision check turns that
coupling from a crash into a named degradation, but every application migration still owes
WeightRoomGym a release, and the suite now has one component whose test suite needs four others'
fixtures. The traceability matrix gains rows whose owner is "WeightRoomGym (reads FreeWeight)" and
the like; those rows are the coupling made visible.

*Negative.* A compromised WeightRoomGym session is a compromised machine — the same blast radius
...
Gold Standards §1.1 gains WeightRoomGym's dependency row; Gold Standards §2 gains its section.
...
* **Package-shaped WeightRoomGym that applications embed.** Rejected on
...
* **An application grows an administrative API that covers something WeightRoomGym reads directly.**
  The API wins, the direct read is retired, and rule 2's list shrinks — the exception should only
  ever narrow.
* **A second machine.** Rule 2 is written for one host; a WeightRoomGym reading databases over a
