...
def test_every_page_still_renders_with_every_application_stopped(stopped: Console) -> None:
    ...
    # The Overview page's log tail is the same `_log_pane.html` the unified /logs page uses:
    # MirrorWall's `log_pane` swaps each frame's data in as-is, and these frames are JSON.
    assert 'data-log-stream="/api/v1/apps/loadcoach/logs/stream"' in page
    assert "sse-swap" not in page
